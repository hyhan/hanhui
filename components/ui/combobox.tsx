...
export const Combobox = React.forwardRef(
  (props: ComboboxProps, ref: React.ForwardedRef<HTMLInputElement>) => {
    ...
    const tagMap = React.useMemo(() => {
      return new Map<string, string>(props?.options?.map((el) => [el.value, el.label]) ?? [])
    }, [props?.options])
          ...
          >
            ...
            >
              ...
              {props.multiple &&
                ...
                props.value?.map((el) => <Badge key={el}>{tagMap.get(el)}</Badge>)}
            ...
            <div className="flex h-full items-center shrink-0">
              ...
              {props.multiple && (
                <XIcon
                  className={cn('ml-2 h-4 w-4 opacity-50 hover:opacity-80 transition-opacity')}
                  onClick={(e) => {
                    props.onValueChange?.([])
